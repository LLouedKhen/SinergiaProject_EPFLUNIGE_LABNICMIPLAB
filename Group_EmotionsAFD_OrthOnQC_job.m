...
GAnDir = 'SecondLevel_EmotionsAFD_OrthOnQC/';
    if ~exist(GAnDir, 'dir')
        mkdir(GAnDir);
    end

cd SecondLevel_EmotionsAFD_OrthOnQC/
for em = 1:length(Emotions) 
    ...
    files =  dir(fullfile(imgBetaPath, '**', ['FullModel__','*','_AllPmod_AFD_OrthOn'], ['con_00',emNum,'.nii']));
    
    cFiles = {};
     for f = 1:length(files)
         thisStr = fullfile(files(f).folder,files(f).name);
         thisMov = extractBetween(thisStr,'FullModel__','_AllPmod_AFD_OrthOn');
         thisMov = thisMov{1};
%          thisCon = thisCon{1};
