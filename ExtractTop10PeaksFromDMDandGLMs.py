...
clFiles = sorted(glob.glob('*peaks.csv'))
indexDFMu = ['MU'] * 10
indexDFFD = ['FD'] * 10
indexFM = ['FirstM'] * 10
indexFirstCpxr = ['FirstCpxr'] * 10
indexFirstCpxi = ['FirstCpxi'] * 10
columnsDF = ['cluster_id', 'peak_x', 'peak_y', 'peak_z', 'peak_value', 'volume_mm', 'aal', 'Emotion']
allData = []
for f in range(len(clFiles)):
    ...
    if 'FWE' in x:
        ...
        s1 = x.split('FirstConj_i_')[1].split('_peaks.csv')[0]
        em = s1.split('_')[1]
        df = data.iloc[:nClust, :]
        ...
        s1 = x.split('FirstConj_r_')[1].split('_peaks.csv')[0]
        em = s1.split('_')[1]
        df = data.iloc[:nClust, :]
        ...
        em = x.split('FirstMode_')[1].split('_peaks.csv')[0]
        df = data.iloc[:nClust, :]
