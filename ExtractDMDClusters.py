...
#    create_output(bFile1, cluster_extent=5, atlas=['aal'], voxel_thresh=1)
#
...
#    create_output(bFile2, cluster_extent=5, atlas=['aal'], voxel_thresh=1)
##    create_output(bFile2, cluster_extent=5, atlas=['destrieux'], voxel_thresh=0.001
#    
          
#    c = plotting.plot_stat_map(filename3)
    coordComp.MaxCoord.iloc[2] = np.round(plotting.find_xyz_cut_coords(filename3, activation_threshold = 0.0001))
#    coordComp.MaxCoord.iloc[2] = np.round(c.cut_coords, 2)
    create_output(filename3, cluster_extent=1, atlas=['aal'], voxel_thresh=0.0001)
    
        
#    d = plotting.plot_stat_map(filename1)
    #coordComp.MaxCoord.iloc[3] = np.round(d.cut_coords, 2)
    coordComp.MaxCoord.iloc[3] = np.round(plotting.find_xyz_cut_coords(filename1, activation_threshold = 0.0001))
    create_output(filename1, cluster_extent=1, atlas=['aal'], voxel_thresh=0.0001)
#       
#    e = plotting.plot_stat_map(filename2)
    #coordComp.MaxCoord.iloc[4] = np.round(e.cut_coords, 2)
    coordComp.MaxCoord.iloc[4] = np.round(plotting.find_xyz_cut_coords(filename2, activation_threshold = 0.0001))
    create_output(filename2, cluster_extent=1, atlas=['aal'], voxel_thresh=0.001)
