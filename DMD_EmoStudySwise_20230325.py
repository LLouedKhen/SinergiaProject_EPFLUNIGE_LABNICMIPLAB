...
for em in Emotions:
    os.chdir(dmdPath)
    os.chdir(em)
    files = glob.glob('BetaSeriesSubjectWise*')
    files = sorted(files)
    data3 = np.array([])
    for f in range (len(files)):
            ...
            if em in thisFile:
                str1 = thisFile
                start = 'BetaSeriesSubjectWise_'
                end = '_'
                thisSub = (str1.split(start))[1].split(end)[0]
...
#                    else:
                ...
                dcp = Decomposition(data1, filenames ='/home/loued/.local/lib/python3.7/site-packages/nidmd/tests/data/schaefer.csv' )
    #dcp = Decomposition(data3)
                ts = TimeSeries(data = data1, sampling_time = 1.3)
    ...
    #dmd.keys(), dmd['values'][:5]
                ...
                dfAll =dcp.df
                dfAll.to_csv('DMD_' + em + ' _' + thisSub + '.csv')
