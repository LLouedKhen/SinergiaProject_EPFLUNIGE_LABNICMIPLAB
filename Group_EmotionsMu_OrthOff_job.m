...
GAnDir = 'SecondLevel_EmotionsMu_OrthOff/';
    if ~exist(GAnDir, 'dir')
        mkdir(GAnDir);
    end

cd SecondLevel_EmotionsMu_OrthOff/
for em = 1:length(Emotions) 
    ...
    files =  dir(fullfile(imgBetaPath, '**', ['FullModel__','*','_AllPmod_MU_OrthOff'], ['con_00',emNum,'.nii']));
    
    cFiles = {};
     for f = 1:length(files)
         thisStr = files(f).name;
         thisMov = extractBetween(thisStr,'FullModel__','_AllPmod_MU_OrthOff');
