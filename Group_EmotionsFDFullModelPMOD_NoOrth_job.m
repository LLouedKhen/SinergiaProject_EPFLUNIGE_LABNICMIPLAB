...
GAnDir = 'SecondLevel_EmotionsFD_AllPMODNoOrth';
    if ~exist(GAnDir, 'dir')
        mkdir(GAnDir);
    end

cd SecondLevel_EmotionsFD_AllPMODNoOrth/
for em = 1:length(Emotions) 
    ...
    files =  dir(fullfile(imgBetaPath, '**', ['FullModel_','*', '_AllPmod_FD_OrthFree'], ['con_00',emNum,'.nii']));
