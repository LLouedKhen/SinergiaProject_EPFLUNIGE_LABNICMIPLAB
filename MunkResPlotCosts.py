...
inPath = '/media/miplab-nas2/Data2/Movies_Emo/Leyla/DMD_Data/MovieEmotions/SubjectWise/MunkResOut'
...
for em in Emotions:
    ...
    for s in Subjects:
        thisS = []       
        filesSubEm = glob.glob('*'+s +'*'+em+'*' )
        for m in range(19):
            mIdx = filesSubEm.index('MunkRes_' + s + '_' + em + '_Mode' +str(m)+'.csv')
            thisData = pd.read_csv(filesSubEm[mIdx])
            z = thisData.cost[0]
            if isinstance(z, str):
                c1 = z.replace("i", "j")
                c2 = c1.replace(" ", "")
...
for ax1 in axr.flat:
    ...
    plt.savefig('RealCostbySubjectEmotiontoS1.png', format="png")
    os.chdir(inPath)
...
#    fig2.delaxes(axi[3][2])
    ...
    os.chdir('/home/loued/Figures')
    plt.savefig('ImagCostbySubjectEmotiontoS1.png', format="png")
    os.chdir(inPath)        
