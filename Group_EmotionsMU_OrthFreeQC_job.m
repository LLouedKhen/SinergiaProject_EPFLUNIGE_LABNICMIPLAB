...
GAnDir = 'SecondLevel_EmotionsMU_OrthFreeQC/';
    if ~exist(GAnDir, 'dir')
        mkdir(GAnDir);
    end

cd SecondLevel_EmotionsMU_OrthFreeQC/
for em = 1:length(Emotions) 
    ...
    files =  dir(fullfile(imgBetaPath, '**', ['FullModel__','*','_AllPmod_MU_OrthFree'], ['con_00',emNum,'.nii']));
    
    cFiles = {};
     for f = 1:length(files)
         thisStr = fullfile(files(f).folder,files(f).name);
         thisMov = extractBetween(thisStr,'FullModel__','_AllPmod_MU_OrthFree');
         thisMov = thisMov{1};
%          thisCon = thisCon{1};
