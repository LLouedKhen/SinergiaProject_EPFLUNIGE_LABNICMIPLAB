...
cd(bPath)
relFiles = dir('atlas_neuromorphometrics_*.nii');
...
% sFolders = sFolders(keep);
 ...
 for i = 1:length(sFolders)
    ...
    cd(thisFolder)
    sModels = dir('FullModel__*AllPmod_FD_OrthOn');
    s = 0;
    for s = 1:length(sModels)
        ...
        st = sModels(s).name;
        dirName = extractBetween(st, 'FullModel__', 'AllPmod_FD_OrthOn');
        dirName = 'NeuroMorph_BetaSeries';
...
%                         matlabbatch{2}.cfg_basicio.var_ops.cfg_save_vars.saveasstruct = true;
                    ...
                    cd(opDir)
                    fileID = fopen(['DoneInMayNMPH',Emotions{em},'.txt'],'w');
                    fclose(fileID);
